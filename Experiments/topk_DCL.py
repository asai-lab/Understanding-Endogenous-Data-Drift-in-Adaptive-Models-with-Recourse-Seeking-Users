...
class Exp3(Helper):
    ...
    def update(self, model: nn.Module, train: Dataset, sample: Dataset, recoursedFail, recoursedSuccess):
        ...
        if self.round != 1:
            ...
            if(self.jsd_list == []):
                # if no previous jsd value, set tao to 0
                continual_training(self.si, self.train, 10, tao = 0, observe_range=7)
            else:
                continual_training(self.si, self.train, 10, tao = 0.00001/(self.jsd_list[-1]), observe_range=7)
